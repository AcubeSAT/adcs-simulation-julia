...
function residual_magnetic_torque(Area, cosines, B_body)
    mean_rm_base = [0.05, 0.05, 0.05]
    maximum_area = [0.034, 0.034, 0.01]
    
    sign_vector = sign.(-cosines)
    
    rm = mean_rm_base .+ (0.005 .* (Area ./ maximum_area) .* sign_vector) .+ 0.0005 .* rand(3)
...
#    Ouputs:
#      solar_tor     - Solar pressure torque
#      area          - Satellite's area projected to the sun
...
function solar_radiation_pressure(R_OB, sun_vector, cm)
    ...
    # Change: use of dot function, instead of (*)
    ...
    # Change: use 0.34 and 0.1 instead of 0.034 and 0.01
    ...
    # What about the cos(i) from the DDJF and why the use of cm
